...
// IMPORTANT: every readFileSync path below must be a literal string, not built from a
// variable/template. Vercel's build-time file tracer only bundles files it can resolve
// statically — a dynamic path (e.g. `../assets/fonts/${name}`) gets silently left out of
// the deployed function, so the file exists in the repo but 404s (ENOENT) in production.
...
const FONTS={
  agrandir:dataUri(readFileSync(new URL('../assets/fonts/agrandir-tight.ttf',import.meta.url)),'font/ttf'),
  agrandirBold:dataUri(readFileSync(new URL('../assets/fonts/agrandir-tight-bold.ttf',import.meta.url)),'font/ttf'),
  agrandirValuesBold:dataUri(readFileSync(new URL('../assets/fonts/agrandir-tight-values-bold.ttf',import.meta.url)),'font/ttf'),
};
const PSM_PAGES={
  adulto:[
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-01.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-02.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-03.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-04.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-05.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-06.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-07.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-08.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-09.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/adulto/page-10.png',import.meta.url)),'image/png'),
  ],
  infantil:[
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-01.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-02.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-03.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-04.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-05.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-06.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-07.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-08.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-09.png',import.meta.url)),'image/png'),
    dataUri(readFileSync(new URL('../assets/psm/infantil/page-10.png',import.meta.url)),'image/png'),
  ],
};
...
function documentPdfHtml(item){
  ...
  const brand=`<div class="doc-preview-brand"><div class="brand-lockup"><img class="brand-symbol" src="${brandSymbolWhiteUri}" alt=""><div class="brand-copy"><div class="brand-name"><strong>Jaqueline</strong><br>Vieira</div><div class="brand-role"><strong>PSICÓLOGA</strong><span>CRP 06/191478</span></div></div></div></div>`;
  const body=`${brand}<h2>${escHtml(item.titulo)}</h2>${campos}${secoes}${closing}`;
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escHtml(item.titulo)}</title><style>
@font-face{font-family:Agrandir;src:url('${FONTS.agrandir}')}
@font-face{font-family:Agrandir;src:url('${FONTS.agrandirBold}');font-weight:700}
@page{size:A4}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}
body{font:10.5pt Arial;line-height:1.55;color:#333;margin:0;background:#fbfaf9}
.doc-preview-brand{height:29mm;margin:0 0 12mm;position:relative;overflow:visible}
.doc-preview-brand:before{content:'';position:absolute;left:-5mm;top:-8.5mm;width:42mm;height:42mm;border-radius:50%;background:${colorido?'#b7c3a5':'#b8b8b8'};z-index:0}
.doc-preview-brand:after{content:'';position:absolute;left:0;bottom:0;width:48mm;border-bottom:2px solid #9aa7aa;z-index:1}
.brand-lockup{position:absolute;left:1.8mm;top:1.5mm;display:flex;align-items:center;color:#6b6b70;line-height:.82;z-index:2}
.brand-symbol{width:27mm;height:23mm;object-fit:contain}
.brand-copy{font-family:Agrandir,Arial Narrow,sans-serif;margin-left:9mm}
.brand-name{font-size:16pt}
.brand-role{display:flex;gap:2.4mm;margin-top:1.7mm;font-size:5.3pt}
.brand-role strong,.brand-name strong{font-weight:700}
h2{text-align:center;color:#626267;font:700 15pt Agrandir;margin:10mm 0 15mm}
h3{color:#626267;margin-top:5mm;break-after:avoid}
p{text-align:justify;white-space:pre-wrap;orphans:3;widows:3}
.doc-closing{break-inside:avoid}
.doc-signature{position:relative;text-align:right;margin-top:18mm;padding-top:4mm;padding-right:27mm;min-height:31mm}
.doc-signature:before{content:'';position:absolute;right:0;top:0;width:88mm;border-top:2px solid #9aa7aa}
.doc-signature img{position:absolute;right:0;bottom:0;width:24mm;height:27mm;object-fit:contain;opacity:.28}
.doc-signature p{margin:0;text-align:right}
.doc-signature strong{color:#69696e}
${colorido?'':'body{filter:grayscale(1)}'}
...
function psmPdfHtml(publico,individual,pacote){
  ...
  const pages=PSM_PAGES[publico]||[];
  const slides=pages.map((img,i)=>{
...
@font-face{font-family:Agrandir;src:url('${FONTS.agrandirValuesBold}');font-weight:700}
...
export async function handleDocuments(req,res,user){
  try{
    ...
    if(req.method==='GET'&&req.query?.action==='export-pdf'){
      ...
      const buffer=await renderPdf(documentPdfHtml(item),{format:'A4',margin:{top:'14mm',right:'18mm',bottom:'14mm',left:'18mm'}});
  ...
  }catch(error){console.error('[documentos] falha:',error);return res.status(500).json({error:'Não foi possível processar o documento'});}
}
