...
// the client's constraint -- one deck, two skins). TCC for children starts
// with naming a feeling, so each infantil slide carries one original
// "feelings" mascot (never traced from any existing character -- simple
// abstract line marks in the logo's own stroke style) in a pastel color,
// cycling through five feelings across the seven content slides. The cover
// introduces all five together, like a cast list at the start of a book.
const PSM_EMOTIONS=[
  {name:'Alegria',bg:'#fbf0c8',fg:'#c99a2e',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><circle cx="12" cy="12" r="4"/><path d="M12 3v3M12 18v3M3 12h3M18 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M18.4 5.6l-2.1 2.1M7.7 16.3l-2.1 2.1"/></svg>'},
  {name:'Tristeza',bg:'#e4eef7',fg:'#6e9dc4',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3c3 4 6 7.5 6 11a6 6 0 0 1-12 0c0-3.5 3-7 6-11z"/></svg>'},
  {name:'Raiva',bg:'#fbe4e1',fg:'#d97066',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l2 6 6 1-5 4 1.5 6L12 17l-4.5 3L9 14 4 10l6-1z"/></svg>'},
  {name:'Medo',bg:'#eee6f7',fg:'#a784cc',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4C8 4 5 7.5 5 11.5A6.5 6.5 0 0 0 11.5 18c1.7 0 3-1 3-2.3S13.3 14 12 14s-2-.8-2-2 1.3-2.7 3.3-2.7 3.7 1.7 3.7 4"/></svg>'},
  {name:'Nojinho',bg:'#e7f2e0',fg:'#7fa968',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4C8 4 5 8 5 12c2 0 4-1 5-3 0 3-2 5-2 8 3 0 6-3 6-7 0-2-1-4-2-6z"/></svg>'},
];
function psmHeader(titulo,publico,emoIdx){
  if(publico==='infantil'){
    const e=PSM_EMOTIONS[emoIdx%PSM_EMOTIONS.length];
...
function psmCoverHtml(publico){
  ...
  const cast=publico==='infantil'?`<div class="psm-cover-cast">${PSM_EMOTIONS.map(e=>`<span class="psm-icon-badge psm-emo-badge" style="--emo:${e.fg};--emo-bg:${e.bg}">${e.icon}</span>`).join('')}</div>`:'';
...
.psm-cover-cast{position:absolute;right:1in;bottom:1in;display:flex;gap:.16in}
.psm-cover-cast .psm-icon-badge{width:.62in;height:.62in}
.psm-cover-cast .psm-icon-badge svg{width:.32in;height:.32in}
...
.psm-kids .psm-icon-badge{background:var(--emo-bg);border:1.5px solid var(--emo);color:var(--emo);border-radius:50%;padding:.16in}
.psm-kids .psm-icon-badge svg{color:var(--emo)}
.psm-emo-badge{width:.55in;height:.55in;flex-shrink:0}
.psm-emo-badge svg{width:.3in;height:.3in;display:block}
