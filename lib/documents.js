...
@font-face{font-family:Agrandir;src:url('${fontUri('agrandir-tight.ttf')}')}
@font-face{font-family:Agrandir;src:url('${fontUri('agrandir-tight-bold.ttf')}');font-weight:700}
@page{size:A4}
...
function prontuarioPdfHtml(nome,sessoes){
  ...
  const brand=`<div class="doc-preview-brand"><div class="brand-lockup"><img class="brand-symbol" src="${brandSymbolWhiteUri}" alt=""><div class="brand-copy"><div class="brand-name"><strong>Jaqueline</strong><br>Vieira</div><div class="brand-role"><strong>PSICÓLOGA</strong><span>CRP 06/191478</span></div></div></div></div>`;
  const body=`${brand}<h2>Prontuário — ${escHtml(nome)}</h2><table class="pront-table"><thead><tr><th class="pront-data">Data</th><th class="pront-relato">Relato da sessão</th></tr></thead><tbody>${linhas}</tbody></table>`;
  return `<!doctype html><html><head><meta charset="utf-8"><title>Prontuário - ${escHtml(nome)}</title><style>
@font-face{font-family:Agrandir;src:url('${fontUri('agrandir-tight.ttf')}')}
@font-face{font-family:Agrandir;src:url('${fontUri('agrandir-tight-bold.ttf')}');font-weight:700}
@page{size:A4}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}
body{font:10.5pt Arial;line-height:1.55;color:#333;margin:0;background:#fbfaf9}
.doc-preview-brand{height:29mm;margin:0 0 12mm;position:relative;overflow:visible}
.doc-preview-brand:before{content:'';position:absolute;left:-5mm;top:-8.5mm;width:42mm;height:42mm;border-radius:50%;background:#b7c3a5;z-index:0}
.doc-preview-brand:after{content:'';position:absolute;left:0;bottom:0;width:48mm;border-bottom:2px solid #9aa7aa;z-index:1}
.brand-lockup{position:absolute;left:1.8mm;top:1.5mm;display:flex;align-items:center;color:#6b6b70;line-height:.82;z-index:2}
.brand-symbol{width:27mm;height:23mm;object-fit:contain}
.brand-copy{font-family:Agrandir,Arial Narrow,sans-serif;margin-left:9mm}
...
.brand-role{display:flex;gap:2.4mm;margin-top:1.7mm;font-size:5.3pt}
.brand-role strong,.brand-name strong{font-weight:700}
h2{text-align:center;color:#626267;font:700 15pt Agrandir;margin:10mm 0 10mm}
.pront-table{width:100%;border-collapse:collapse;break-inside:auto}
.pront-table th{background:#b7c3a5;color:#fff;text-align:left;padding:6px 10px;font-size:9.5pt}
.pront-table td{border:1px solid #ddd;padding:6px 10px;vertical-align:top;font-size:9.5pt}
.pront-data{white-space:nowrap;width:70pt;color:#555}
.pront-table tr{break-inside:avoid}
</style></head><body>${body}</body></html>`;
}

export async function renderProntuarioPdf(nome,sessoes){
  const {renderPdf}=await import('./pdf.js');
  return renderPdf(prontuarioPdfHtml(nome,sessoes),{format:'A4',margin:{top:'14mm',right:'18mm',bottom:'14mm',left:'18mm'}});
}
