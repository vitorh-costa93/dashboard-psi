    ...
    for (const t of TIPOS) {
      ...
      for (let i = 0; i < (Number(s[t.key]) || 0); i++) {
        rows.push({
          ...
          'Valor total': valor, 'Valor final': valor, 'Comparecimento': t.falta ? 'Não' : 'Sim', 'Comentário': '',
        });
      }
    }
  }
  for (const h of historico) {
    ...
    sextas.forEach((iso, i) => {
      ...
      rows.push({
        ...
        'Valor histórico': parte, 'Só valor': true, 'Comparecimento': '', 'Comentário': '',
      });
    });
  }
  return rows;
...
// Linhas resumidas para "Atendimentos — detalhado": uma por semana (sessões =
// realizados + acolhimentos, sem as faltas; valor = total da semana) e uma por
// mês do histórico. Somente leitura.
export function linhasDetalhadas(semanas, historico, pacienteId, {month = '', tarifas = TARIFAS} = {}) {
  ...
  const q = (s, k) => Number(s[k]) || 0;
  const out = [];
  for (const s of semanas) {
    if (!dentro(s.semana_ref)) continue;
    const faltas = q(s, 'faltas'), acolh = q(s, 'acolh_antes') + q(s, 'acolh_apos'), real = q(s, 'real_antes') + q(s, 'real_apos');
    if (!faltas && !acolh && !real) continue;
    out.push({
      ...base, id: `wellz:${s.semana_ref}`, source_key: `wellz:${s.semana_ref}`, data_sessao: s.semana_ref,
      comparecimento: 'Sim', sessoes_cobradas: acolh + real, valor_sessao: 0,
      valor_final: valorSemana(s, tarifas), comentario: `${faltas} falta(s) · ${acolh} acolhimento(s) · ${real} realizado(s)`,
    });
  }
  for (const h of historico) {
