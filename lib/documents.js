...
function documentPdfHtml(item){
  ...
  const campos=item.tipo==='declaracao'?'':Object.entries(fields).filter(([,v])=>clean(v)).map(([k,v])=>`<p><strong>${escHtml(k)}:</strong> ${escHtml(dataBR(v))}</p>`).join('');
...
function documentChildren(item){
  ...
  if(item.tipo!=='declaracao')Object.entries(fields).filter(([,v])=>clean(v)).forEach(([k,v])=>out.push(new Paragraph({spacing:{after:100},children:[new TextRun({text:`${k}: `,bold:true,font:'Arial',size:21}),new TextRun({text:formatDates(v),font:'Arial',size:21})]})));
