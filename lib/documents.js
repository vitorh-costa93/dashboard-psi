...
export async function handleDocuments(req,res,user){
  try{
    ...
    if(req.method==='POST'&&req.body?.action==='generate'){
      ...
      // sempre nas 4 seções fixas; "orientacao" é um texto corrido mais
      ...
      // frontend só quebra em seções quando o formato é estruturado.
      const formato=clean(req.body.formato)==='orientacao'?'orientacao':'estruturado';
      const instructions=(tipo==='relatorio_psicologico'
        ? (formato==='orientacao'
          ? 'Redija uma orientação psicológica profissional em português do Brasil: um texto corrido, direto e curto (no máximo 3 a 4 parágrafos curtos), sem dividir em seções tituladas (nada de "Descrição/Análise/Conclusão/Orientações" como títulos). Traga o contexto essencial em uma ou duas frases e o restante deve ser a orientação prática em si. Não invente diagnósticos, fatos ou dados. Use somente o relato fornecido. Transforme falas coloquiais, ofensivas ou citadas entre aspas em discurso indireto, técnico, respeitoso e adequado ao documento. Não reproduza insultos, palavrões ou falas literais entre aspas; descreva objetivamente o conteúdo e o contexto informado.'
      ...
      const input=historico.length
        ...
        : descricao;
      const r=await fetchComRetentativa('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5-mini',instructions,input,max_output_tokens:3000,reasoning:{effort:'low'}})},{tentativas:2,timeoutMs:30000});
