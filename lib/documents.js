...
const PSM_WAVE='<svg class="psm-wave" viewBox="0 0 120 14" preserveAspectRatio="none"><path d="M0,8 C10,1 18,1 28,8 C38,15 46,15 56,8 C66,1 74,1 84,8 C94,15 102,15 112,8 L120,6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
...
function psmHeader(titulo){
  return `<div class="psm-content-head"><h2>${escHtml(titulo)}</h2>${PSM_WAVE}</div>`;
}
function psmAbordagemHtml(publico){
  const t=PSM_TEXT[publico]||PSM_TEXT.adulto;
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Abordagem utilizada')}
...
function psmOnlineHtml(){
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Terapia online')}
...
<div class="psm-req-item">${ICON_WIFI}<span>Internet com<br>boa conexão</span></div>
<div class="psm-req-item">${ICON_HOME}<span>Lugar confortável<br>e reservado</span></div>
</div></div>`;
}
function psmPresencialHtml(publico){
  const t=PSM_TEXT[publico]||PSM_TEXT.adulto;
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Terapia presencial')}
<p class="psm-content-lead">As sessões duram <strong>50 min</strong> e são realizadas no seguinte endereço:</p>
<div class="psm-address-block">${ICON_HOME}<div class="psm-address-text"><strong>${escHtml(t.endereco.nome)}</strong>${t.endereco.linhas.map(l=>`<span>${escHtml(l)}</span>`).join('')}</div></div>
</div>`;
}
function psmListHtml(titulo,items){
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader(titulo)}
<ul class="psm-list">${items.map(i=>`<li>${PSM_SPRIG}<span>${escHtml(i)}</span></li>`).join('')}</ul>
</div>`;
}
function psmValoresHtml(individual,pacote){
  ...
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Valores')}
...
function psmAcompanheHtml(){
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Me acompanhe')}
<div class="psm-photo-wrap"><img class="psm-photo" src="${wifePhotoUri}" alt="Jaqueline Vieira"></div>
<div class="psm-insta"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.2" cy="6.8" r="1"/></svg><span>@psi_jaquelinevieira</span></div>
</div>`;
}
function psmAgendamentoHtml(){
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-bl" src="${leafBottomLeftUri}" alt="">${psmHeader('Agendamento')}
...
function psmInnerSlideHtml(index,publico,individual,pacote){
  ...
  if(index===2)return psmOnlineHtml();
  if(index===3)return psmPresencialHtml(publico);
  if(index===4)return psmListHtml('Observações de agendamento',t.agendamento);
  if(index===5)return psmListHtml('Observações de pagamento',PSM_PAGAMENTO);
  if(index===6)return psmValoresHtml(individual,pacote);
  if(index===7)return psmAgendamentoHtml();
  return '';
}
function psmCoverHtml(publico){
  const subtitle=publico==='infantil'?'<div class="psm-cover-sub">Psicoterapia Infantil</div>':'';
  return `<div class="psm-cover">
...
function psmPdfHtmlNew(publico,individual,pacote){
  ...
  const slides=Array.from({length:10},(_,i)=>{
    ...
    if(i===8)return `<section class="psm-slide psm-slide-html">${psmAcompanheHtml()}</section>`;
...
.psm-content-head{margin-bottom:.55in;flex-shrink:0}
.psm-content-head h2{font:300 38pt/1.15 'Fraunces',serif;font-style:italic;color:#4e553e;margin:0 0 .2in;letter-spacing:.002em}
.psm-wave{display:block;width:1.1in;height:.13in;color:#899776;opacity:.85}
...
.psm-list{margin:0;padding:0;list-style:none;display:flex;flex-direction:column;gap:.28in}
.psm-list li{display:flex;align-items:flex-start;gap:.24in;font:400 15pt/1.6 'Newsreader',serif;color:#4a4234}
.psm-sprig{width:.22in;height:.22in;color:#899776;flex-shrink:0;margin-top:.09in}
...
.psm-photo-wrap{display:flex;justify-content:center;margin:.2in 0 .5in}
.psm-photo{width:2.6in;height:2.6in;border-radius:50%;object-fit:cover;border:1.5px solid #b86a49;box-shadow:0 0 0 8px #f5f1e8,0 0 0 9px #dccfb4}
