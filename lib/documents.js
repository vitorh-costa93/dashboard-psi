...
function documentChildren(item){
  ...
  const out=[
    new Table({width:{size:100,type:WidthType.PERCENTAGE},borders:noBorders,rows:[new TableRow({children:[
      new TableCell({width:{size:34,type:WidthType.PERCENTAGE},shading:{fill:c.estilo==='preto-branco'?'777777':'78845C'},verticalAlign:VerticalAlign.CENTER,margins:{top:80,bottom:80,left:100,right:100},children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new ImageRun({data:brandSymbolWhite,transformation:{width:76,height:85},type:'png',altText:{title:'Símbolo Jaqueline Vieira',description:'Flor da identidade visual',name:'Símbolo'}})]})]}),
      new TableCell({width:{size:66,type:WidthType.PERCENTAGE},verticalAlign:VerticalAlign.CENTER,margins:{top:70,bottom:60,left:220,right:80},children:[new Paragraph({spacing:{after:0},children:[new TextRun({text:'Jaqueline',bold:true,font:'Arial Narrow',size:34,color:'66666B'})]}),new Paragraph({spacing:{after:90},children:[new TextRun({text:'Vieira',font:'Arial Narrow',size:32,color:'66666B'})]}),new Paragraph({children:[new TextRun({text:'PSICÓLOGA   ',bold:true,font:'Arial Narrow',size:16,color:'66666B'}),new TextRun({text:'CRP 06/191478',font:'Arial Narrow',size:16,color:'66666B'})]})]})
    ]})]}),
