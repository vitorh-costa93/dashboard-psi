...
async function psmInfantilWebpPdf(individual,pacote){
  const valorBR=v=>{const n=Number(String(v||'').replace(',','.'));return Number.isFinite(n)?n.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}):'';};
  const renderKidsPage7=async(webpBuffer)=>{
    const base=sharp(webpBuffer);
    const meta=await base.metadata();
    const width=meta.width||2430, height=meta.height||1620;
    const overlay=sharp({create:{width,height,channels:4,background:{r:0,g:0,b:0,alpha:0}}});
    const fontPx=Math.round(width*0.0337); // ~82px at 2430
    const y=Math.round(height*0.4444);      // tuned to sit inside the yellow cards
    const centers=[Math.round(width*0.2778),Math.round(width*0.6152)];
    const values=[`R$${valorBR(individual)}`,`R$${valorBR(pacote)}`];
    const comps=[];
    for(let idx=0;idx<2;idx++){
      const txt=`<span foreground="#008884">${values[idx]}</span>`;
      const textImg=sharp({
        text:{
          text:txt,
          font:`DejaVu Sans Bold ${fontPx}`,
          rgba:true,
          dpi:300,
          align:'center'
        }
      }).png();
      const info=await textImg.metadata();
      const buf=await textImg.toBuffer();
      comps.push({input:buf,left:Math.round(centers[idx]-((info.width||0)/2)),top:y});
    }
    const overlayBuf=await overlay.composite(comps).png().toBuffer();
    return await base.composite([{input:overlayBuf}]).jpeg({quality:95}).toBuffer();
  };
  const pdf=await PDFDocument.create();
  for(let i=1;i<=10;i++){
    const n=String(i).padStart(2,'0');
    const webp=readFileSync(new URL(`../assets/psm/infantil/emocoes-page-${n}.webp`,import.meta.url));
    const jpg=i===7?await renderKidsPage7(webp):await sharp(webp).jpeg({quality:95}).toBuffer();
