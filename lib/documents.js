...
function psmOnlineHtml(){
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-bl" src="${leafBottomLeftUri}" alt="">${psmHeader('Terapia online')}
...
function psmListHtml(titulo,items,leaf){
  ...
  const rot=[-6,4,-3,7,-5];
  return `<div class="psm-content">${img}${psmHeader(titulo)}
...
function psmValoresHtml(individual,pacote){
  ...
  return `<div class="psm-content"><img class="psm-leaf psm-leaf-bl" src="${leafBottomLeftUri}" alt="">${psmHeader('Valores')}
...
function psmInnerSlideHtml(index,publico,individual,pacote){
  ...
  if(index===4)return psmListHtml('Observações de agendamento',t.agendamento,'tr');
  if(index===5)return psmListHtml('Observações de pagamento',PSM_PAGAMENTO,'bl');
