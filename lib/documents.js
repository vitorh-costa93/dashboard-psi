...
function psmPdfHtmlKidsEmotion(individual,pacote){
  ...
  const asset=name=>dataUri(readFileSync(new URL(`../assets/psm/infantil-emocoes/ui/${name}`,import.meta.url)),'image/png');
  const A={cover:asset('cover-illustration.png'),tcc:asset('tcc-diagram.png'),online:asset('online-illustration.png'),presencial:asset('presencial-scene.png'),agenda:asset('agendamento-mascot.png'),pagamento:asset('pagamento-mascot.png'),valores:asset('valores-mascot.png'),contato:asset('contato-illustration.png'),social:asset('instagram-illustration.png'),close:asset('closing-illustration.png')};
  const shell=(cls,n,inner)=>`<section class="slide"><div class="psm-approved ${cls}"><span class="psm-approved-number">${n}</span><i class="blob-bl"></i><i class="blob-br"></i><i class="spark s1">★</i><i class="spark s2">♥</i><i class="spark s3">★</i>${inner}</div></section>`;
  const v1=valorBR(individual)||'R$ 0,00',v4=valorBR(pacote)||'R$ 0,00',regras=PSM_TEXT.infantil.agendamento,pagamentos=PSM_PAGAMENTO;
  const item=(ico,t)=>`<div class="item"><span class="ico">${ico}</span><span>${escHtml(t)}</span></div>`;
  const slides=[
    shell('psm-approved-cover',1,`<div class="psm-approved-cover-copy"><div class="name">Jaqueline<br>Vieira</div><div class="role">Psicóloga Clínica</div><div class="crp">CRP 06/191478</div></div><img class="psm-approved-illus" src="${A.cover}">`),
    shell('psm-approved-tcc',2,`<div class="psm-approved-head"><h2 class="psm-approved-script">Abordagem Utilizada</h2></div><div class="text"><div class="psm-approved-kicker">Terapia Cognitivo-Comportamental</div><div class="psm-approved-body">${PSM_TEXT.infantil.abordagem.map(p=>`<p>${escHtml(p)}</p>`).join('')}</div></div><img class="psm-approved-illus" src="${A.tcc}">`),
    shell('psm-approved-online',3,`<div class="psm-approved-head"><h2 class="psm-approved-script">Terapia Online</h2></div><div class="copy psm-approved-body">As sessões duram <strong>50 min</strong> e são realizadas pelo Google Meet, através de um link criado por mim e enviado na hora agendada, pelo Whatsapp.</div><div class="req-title">Pré-Requisitos</div><div class="reqs"><div class="req"><span class="ico">◔</span>Internet com<br>boa conexão</div><div class="req"><span class="ico">⌂</span>Lugar confortável<br>e reservado</div></div><img class="psm-approved-illus" src="${A.online}">`),
    shell('psm-approved-presencial',4,`<div class="psm-approved-head"><h2 class="psm-approved-script">Terapia Presencial</h2></div><div class="copy psm-approved-body">As sessões duram <strong>50 min</strong> e são realizadas no seguinte endereço:<div class="address"><strong>${escHtml(PSM_TEXT.infantil.endereco.nome)}</strong>${PSM_TEXT.infantil.endereco.linhas.map(x=>`<div>${escHtml(x)}</div>`).join('')}</div></div><img class="psm-approved-illus" src="${A.presencial}">`),
    shell('psm-approved-list',5,`<div class="psm-approved-head"><h2 class="psm-approved-script">Observações agendamento</h2></div><div class="items">${item('◷',regras[0])}${item('▣',regras[1])}${item('♢',regras[2])}${item('!',regras[3])}</div><img class="psm-approved-illus" src="${A.agenda}">`),
    shell('psm-approved-list payment',6,`<div class="psm-approved-head"><h2 class="psm-approved-script">Observações pagamento</h2></div><div class="items">${item('♡',pagamentos[0])}${item('$',pagamentos[1])}${item('▣',pagamentos[2])}${item('≡',pagamentos[3])}</div><img class="psm-approved-illus" src="${A.pagamento}">`),
    shell('psm-approved-values',7,`<div class="psm-approved-head"><h2 class="psm-approved-script">Valores</h2></div><div class="cards"><div class="price"><span>1 sessão</span><strong>${escHtml(v1)}</strong></div><div class="price"><span>Pacote com 4 sessões</span><strong>${escHtml(v4)}</strong></div></div><div class="pay"><h3>Pagamento</h3><div class="pix">Pix: 57.062.579/0001-34</div><p>Gentileza enviar o comprovante de pagamento até o dia da sessão a ser realizada.</p></div><img class="psm-approved-illus" src="${A.valores}">`),
    shell('psm-approved-contact',8,`<div class="psm-approved-head"><h2 class="psm-approved-script">Agendamento</h2></div><div class="copy"><h3>Agendamento e dúvidas</h3><div class="line"><span class="ico">●</span><span>(16) 99746-7348</span></div><div class="line"><span class="ico">✉</span><span>psi.jaquelinev@gmail.com</span></div></div><img class="psm-approved-illus" src="${A.contato}">`),
    shell('psm-approved-social',9,`<div class="psm-approved-head"><h2 class="psm-approved-script">Me Acompanhe</h2></div><div class="handle">◎ @psi_jaquelinevieira</div><img class="psm-approved-illus" src="${A.social}">`),
    shell('psm-approved-close',10,`<div class="thanks">OBRIGADA!</div><div class="name">Jaqueline<br>Vieira</div><div class="badge">PSICÓLOGA</div><div class="crp">CRP 06/191478</div><img class="psm-approved-illus" src="${A.close}">`),
  ].join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>PSM Infantil - Pacote ${escHtml(v4)}</title><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Baloo+2:wght@600;700;800&family=Nunito:wght@600;700;800&display=swap"><style>
@page{size:12in 8in;margin:0}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}body{margin:0}.slide{width:12in;height:8in;position:relative;overflow:hidden;break-after:page}.slide:last-child{break-after:auto}
.psm-approved{position:relative;width:100%;height:100%;overflow:hidden;background:#fff9f0;font-family:'Nunito',sans-serif;color:#282425;container-type:inline-size}.psm-approved:before,.psm-approved:after{content:'';position:absolute;border-radius:45% 55% 60% 40%/48% 45% 55% 52%;opacity:.72}.psm-approved:before{width:21cqw;height:18cqw;left:-8cqw;top:-8cqw;background:#e8ddf4}.psm-approved:after{width:20cqw;height:18cqw;right:-7cqw;top:-8cqw;background:#ffd9ca}.blob-bl{position:absolute;left:-7cqw;bottom:-8cqw;width:21cqw;height:18cqw;border-radius:55% 45% 45% 55%;background:#dcefe5;opacity:.85}.blob-br{position:absolute;right:-8cqw;bottom:-9cqw;width:22cqw;height:19cqw;border-radius:48% 52% 60% 40%;background:#f5ddeb;opacity:.75}.spark{position:absolute;font-size:2.1cqw;line-height:1;z-index:2}.s1{left:27cqw;top:5cqw;color:#e9b82e}.s2{left:32cqw;top:10cqw;color:#e783a0}.s3{left:88cqw;top:7cqw;color:#efc44b}.psm-approved-title{font:800 4.1cqw/1.05 'Baloo 2',cursive;color:#67438b;margin:0}.psm-approved-script{font:800 5.2cqw/.98 'Baloo 2',cursive;color:#67438b;margin:0}.psm-approved-kicker{font:800 2cqw/1.2 'Nunito',sans-serif;color:#13827e}.psm-approved-body{font:650 1.65cqw/1.55 'Nunito',sans-serif;color:#282425}.psm-approved-body strong{font-weight:850}.psm-approved-number{position:absolute;left:2.5cqw;top:2.2cqw;width:4cqw;height:4cqw;border-radius:50%;background:#73528e;color:#fff;display:flex;align-items:center;justify-content:center;font:800 1.7cqw 'Nunito',sans-serif;z-index:4}.psm-approved-illus{position:absolute;z-index:2;object-fit:contain;mix-blend-mode:multiply}.psm-approved-cover-copy{position:absolute;left:6.5cqw;top:23cqw;z-index:3}.psm-approved-cover-copy .name{font:800 7.3cqw/.94 'Baloo 2',cursive;color:#67438b}.psm-approved-cover-copy .role{display:inline-block;margin-top:2.4cqw;padding:1cqw 2.3cqw;border-radius:999px;background:#ef7f9c;color:#fff;font:800 2.25cqw 'Nunito',sans-serif}.psm-approved-cover-copy .crp{margin-top:1.6cqw;text-align:center;color:#13827e;font:800 1.8cqw 'Nunito',sans-serif}.psm-approved-cover .psm-approved-illus{right:0;bottom:-.5cqw;width:58cqw;height:59cqw}.psm-approved-head{position:absolute;left:6.5cqw;top:5.2cqw;z-index:3}.psm-approved-tcc .text{position:absolute;left:5.5cqw;top:20cqw;width:50cqw;z-index:3}.psm-approved-tcc .text .psm-approved-kicker{font-size:2.2cqw;margin-bottom:2.3cqw}.psm-approved-tcc .psm-approved-illus{right:3cqw;top:10cqw;width:39cqw;height:49cqw}.psm-approved-online .copy{position:absolute;left:4.5cqw;top:21cqw;width:46cqw;z-index:3}.psm-approved-online .req-title{position:absolute;left:8cqw;top:55cqw;font:800 3.2cqw 'Baloo 2',cursive;color:#67438b;z-index:3}.psm-approved-online .reqs{position:absolute;left:4.5cqw;bottom:5cqw;display:flex;gap:3cqw;z-index:3}.psm-approved-online .req{width:17cqw;height:16cqw;border-radius:2.3cqw;background:#fff0c7;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;font:700 1.2cqw/1.35 'Nunito',sans-serif}.psm-approved-online .req .ico{font-size:4.5cqw;margin-bottom:1.1cqw;color:#4aa6a0}.psm-approved-online .psm-approved-illus{right:1.5cqw;bottom:0;width:52cqw;height:60cqw}.psm-approved-presencial .copy{position:absolute;left:5cqw;top:22cqw;width:45cqw;z-index:3}.psm-approved-presencial .address{margin-top:2.5cqw;padding:2.5cqw 3cqw;border-radius:2.4cqw;background:rgba(255,250,238,.92)}.psm-approved-presencial .address strong{display:block;color:#67438b;font:800 2.4cqw 'Baloo 2',cursive;margin-bottom:.6cqw}.psm-approved-presencial .psm-approved-illus{right:0;bottom:0;width:49cqw;height:61cqw}.psm-approved-list .items{position:absolute;left:6.5cqw;top:20cqw;width:67cqw;z-index:3;display:flex;flex-direction:column;gap:2cqw}.psm-approved-list .item{display:grid;grid-template-columns:3.4cqw 1fr;gap:1.2cqw;align-items:start;font:650 1.55cqw/1.45 'Nunito',sans-serif}.psm-approved-list .item .ico{width:3.4cqw;height:3.4cqw;border-radius:1.1cqw;background:#fff0d2;display:flex;align-items:center;justify-content:center;font-size:1.8cqw}.psm-approved-list .psm-approved-illus{right:3cqw;bottom:2cqw;width:22cqw;height:26cqw}.psm-approved-list.payment .items{width:70cqw}.psm-approved-list.payment .item .ico{background:#f0e5f8}.psm-approved-list.payment .psm-approved-illus{width:20cqw;height:28cqw;right:2cqw;bottom:1cqw}.psm-approved-values .psm-approved-head{left:31cqw;top:5cqw}.psm-approved-values .cards{position:absolute;left:27cqw;top:21cqw;width:58cqw;display:grid;grid-template-columns:1fr 1fr;gap:3cqw;z-index:3}.psm-approved-values .price{background:#ffefaf;border-radius:2.5cqw;padding:2.7cqw 2cqw;text-align:center}.psm-approved-values .price span{display:block;font:800 1.5cqw 'Nunito',sans-serif}.psm-approved-values .price strong{display:block;margin-top:.7cqw;font:800 3.25cqw 'Baloo 2',cursive;color:#13827e}.psm-approved-values .pay{position:absolute;left:25cqw;top:47cqw;width:62cqw;border-radius:2.7cqw;background:#e1f0e9;padding:2cqw 3cqw;text-align:center;z-index:3}.psm-approved-values .pay h3{margin:0 0 .7cqw;font:800 2.6cqw 'Baloo 2',cursive;color:#67438b}.psm-approved-values .pay .pix{font:800 2.4cqw 'Nunito',sans-serif;color:#13827e}.psm-approved-values .pay p{font:650 1.35cqw 'Nunito',sans-serif;margin:.6cqw 0 0}.psm-approved-values .psm-approved-illus{left:0;bottom:0;width:24cqw;height:38cqw}.psm-approved-contact .copy{position:absolute;left:6cqw;top:22cqw;z-index:3;width:47cqw}.psm-approved-contact .copy h3{font:800 2.5cqw 'Baloo 2',cursive;color:#13827e;margin:0 0 4cqw}.psm-approved-contact .line{display:flex;align-items:center;gap:1.5cqw;margin:2cqw 0;font:700 2cqw 'Nunito',sans-serif}.psm-approved-contact .line .ico{font-size:2.6cqw;color:#45a689}.psm-approved-contact .psm-approved-illus{right:0;bottom:0;width:53cqw;height:61cqw}.psm-approved-social .psm-approved-head{left:7cqw}.psm-approved-social .handle{position:absolute;left:7cqw;top:18cqw;z-index:3;color:#e46082;font:800 2.6cqw 'Nunito',sans-serif}.psm-approved-social .psm-approved-illus{left:21cqw;bottom:-1cqw;width:65cqw;height:59cqw}.psm-approved-close .thanks{position:absolute;left:6cqw;top:12cqw;font:900 7.3cqw/.95 'Baloo 2',cursive;color:#67438b;z-index:3}.psm-approved-close .name{position:absolute;left:8cqw;top:36cqw;font:800 5.4cqw/.95 'Baloo 2',cursive;color:#ef6f96;z-index:3}.psm-approved-close .badge{position:absolute;left:8cqw;top:51cqw;padding:1cqw 2.7cqw;border-radius:999px;background:#70b9b2;color:#fff;font:800 1.6cqw 'Nunito',sans-serif;z-index:3;letter-spacing:.08em}.psm-approved-close .crp{position:absolute;left:10.5cqw;top:58cqw;color:#137d79;font:800 1.5cqw 'Nunito',sans-serif;z-index:3}.psm-approved-close .psm-approved-illus{right:2cqw;bottom:0;width:58cqw;height:59cqw}
