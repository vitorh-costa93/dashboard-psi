  ...
  try{
    ...
    if(req.method==='POST'&&req.body?.action==='generate'){
      ...
      if(req.body.anexo!=null){
        ...
        if(typeof anexo!=='object'||typeof anexo.base64!=='string'||!anexo.base64||typeof anexo.nome!=='string'||!anexo.nome)return res.status(400).json({error:'Anexo inválido'});
        if(anexo.base64.length>6_000_000)return res.status(413).json({error:'Arquivo muito grande. Envie um arquivo de até 4MB.'});
        const nomeArquivo=clean(anexo.nome).slice(0,200),ext=(nomeArquivo.split('.').pop()||'').toLowerCase();
        let buffer;
        try{buffer=Buffer.from(anexo.base64,'base64');}catch{return res.status(400).json({error:'Não foi possível ler o arquivo enviado.'});}
        if(buffer.length>4*1024*1024)return res.status(413).json({error:'Arquivo muito grande. Envie um arquivo de até 4MB.'});
        let textoArquivo='';
        try{
          if(ext==='pdf'){const pdf=await getDocumentProxy(new Uint8Array(buffer));textoArquivo=(await extractPdfText(pdf,{mergePages:true})).text;}
          else if(ext==='docx')textoArquivo=(await mammoth.extractRawText({buffer})).value;
          else if(ext==='txt')textoArquivo=buffer.toString('utf8');
          else return res.status(400).json({error:'Formato não suportado. Envie um arquivo PDF, DOCX ou TXT.'});
        }catch(e){console.error('anexo documents: erro ao extrair texto:',e);return res.status(400).json({error:'Não foi possível ler o conteúdo do arquivo enviado.'});}
        textoArquivo=clean(textoArquivo);
        if(!textoArquivo)return res.status(400).json({error:'Não foi possível extrair texto do arquivo enviado.'});
        if(textoArquivo.length>8000)textoArquivo=textoArquivo.slice(0,8000)+'\n[...texto truncado...]';
        descricaoComAnexo=`Conteúdo do arquivo enviado (${nomeArquivo}):\n"""\n${textoArquivo}\n"""\n\n${descricao||'Considere o conteúdo do arquivo acima ao elaborar o texto.'}`.slice(0,12000);
