...
function psmPdfHtmlKidsApproved(individual,pacote){
  // Infantil nova UI: PDF flat. A página já contém todo o layout.
  // Não aplicar overlays, fontes ou componentes HTML para evitar diferenças
  // entre preview e exportação.
  const slides=Array.from({length:10},(_,i)=>{
    const n=String(i+1).padStart(2,'0');
    const img=dataUri(readFileSync(new URL(`../assets/psm/infantil/emocoes-page-${n}.webp`,import.meta.url)),'image/webp');
    return `<section class="psm-slide"><img src="${img}" alt="PSM infantil, página ${i+1}"></section>`;
  }).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>PSM Infantil</title><style>
@page{size:12in 8in;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}
