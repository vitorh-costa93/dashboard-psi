...
export async function handleDocuments(req,res,user){
  try{
    ...
    if(req.method==='GET'&&req.query?.action==='psm-pdf'){
      ...
      const {renderPdf}=await import('./pdf.js');
      const buffer=publico==='infantil'?await psmInfantilWebpPdf():await renderPdf(psmPdfHtml(publico,individual,pacote,theme),{width:'12in',height:'8in',margin:{top:0,right:0,bottom:0,left:0}});
      res.setHeader('Content-Type','application/pdf');
