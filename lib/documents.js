...
// approximation of it. @page margin is 0 and all spacing is real CSS padding
// so the colored header can bleed to the true page edge like it does on screen.
...
html,body{margin:0}
body{font-family:'DM Sans',Arial,sans-serif;font-size:13px;line-height:1.65;color:#333;background:#fbfaf9;padding:30px 42px}
...
.doc-preview-brand .brand-symbol{width:100px;height:88px;object-fit:contain}
...
function documentPdfHtml(item){
  ...
  const body=`${brandMarkupPdf()}<h2>${escHtml(item.titulo)}</h2>${campos}${secoes}${closing}`;
...
function prontuarioPdfHtml(nome,sessoes){
  ...
  const body=`${brandMarkupPdf()}<h2>Prontuário — ${escHtml(nome)}</h2><table class="pront-table"><thead><tr><th class="pront-data">Data</th><th class="pront-relato">Relato da sessão</th></tr></thead><tbody>${linhas}</tbody></table>`;
  return `<!doctype html><html><head><meta charset="utf-8"><title>Prontuário - ${escHtml(nome)}</title>${pageStyles(true)}<style>
.pront-table{width:100%;border-collapse:collapse;break-inside:auto;margin-top:10px}
...
export async function renderProntuarioPdf(nome,sessoes){
  ...
  return renderPdf(prontuarioPdfHtml(nome,sessoes),{format:'A4',margin:{top:0,right:0,bottom:0,left:0}});
}
...
export async function handleDocuments(req,res,user){
  try{
    ...
    if(req.method==='GET'&&req.query?.action==='export-pdf'){
      ...
      const item=await loadOne(id);if(!item)return res.status(404).json({error:'Documento não encontrado'});
      const {renderPdf}=await import('./pdf.js');
      const buffer=await renderPdf(documentPdfHtml(item),{format:'A4',margin:{top:0,right:0,bottom:0,left:0}});
