...
function psmPdfHtmlKidsApproved(individual,pacote){
  ...
  const slides=Array.from({length:10},(_,i)=>{
    ...
    const img=dataUri(readFileSync(new URL(`../assets/psm/infantil/emocoes-page-${n}.webp`,import.meta.url)),'image/webp');
...
@page{size:12in 8in;margin:0}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}body{margin:0}.psm-slide{width:12in;height:8in;position:relative;overflow:hidden;break-after:page;background:#fff}.psm-slide:last-child{break-after:auto}.psm-slide>img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain}.psm-kids-values{position:absolute;inset:0}.psm-kids-value{position:absolute;color:#0a837f;font:700 31pt AgrandirValues,Arial Narrow,sans-serif;line-height:1;letter-spacing:-.02em;text-align:center;white-space:nowrap}.psm-kids-value.one{left:22.7%;top:44.2%;width:21.4%}.psm-kids-value.four{left:49.1%;top:44.2%;width:24.7%}
</style></head><body>${slides}</body></html>`;
}
...
export function psmPdfHtml(publico,individual,pacote,theme){
  if(publico==='infantil'&&theme!=='legacy')return psmPdfHtmlKidsApproved(individual,pacote);
