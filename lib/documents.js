...
// Mirrors the on-screen .doc-preview rules in index.html (same DM Sans/
// Agrandir Tight fonts, same colors) so the exported PDF matches what's
// shown in the app.
//
// The letterhead (brand + document title) is rendered through Puppeteer's
// native headerTemplate/footerTemplate mechanism instead of living in the
// page's own HTML flow. That's a deliberate choice, not a style preference:
// a header living in the normal document flow only appears once, wherever
// it happens to sit in the content; getting it to repeat on every physical
// page requires Chromium's print pagination to treat it as a page-level
// running header, and headerTemplate is the one API guaranteed to do that
// (it's the same mechanism browsers use for their own "page 1 of 3"-style
// print headers) -- a <thead> row was tried first since table headers are
// *supposed* to repeat per page too, but that didn't reliably trigger here.
// HEADER_HEIGHT reserves matching room at the top of every page so body
// text never overlaps it.
const HEADER_HEIGHT='158px';
export const PAGE_MARGIN={top:HEADER_HEIGHT,right:'42px',bottom:'30px',left:'42px'};
const EMPTY_TEMPLATE='<div></div>';

// Chromium's header/footer template renders synchronously with no chance to
// wait for a downloaded @font-face (the document.fonts.ready wait the main
// page gets doesn't apply here), so text set in a not-yet-loaded custom font
// can render invisible for the entire header's lifetime -- and flexbox
// layout, which the rest of this file relies on freely, did not visibly lay
// out its children in this same constrained context. Both are avoided here
// on purpose: only system fonts ('Arial Narrow', matching the app's own
// documented fallback for the brand font) and a <table> for the logo/name
// row, which is a far more universally supported layout primitive.
//
// The wrapping div is given an explicit height matching HEADER_HEIGHT and
// the same background as the body (#fbfaf9): without that, only the space
// our own content actually occupies gets painted, and whatever's left of
// the reserved margin area shows through as plain white, producing a
// visible seam between the header band and the body below it.
function headerTemplateHtml(title,colorido){
  const circleColor=colorido===false?'#b8b8b8':'#b7c3a5';
  return `<div style="width:100%;height:${HEADER_HEIGHT};box-sizing:border-box;background:#fbfaf9;padding:14px 42px 0;font-family:Arial,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact;">
<table style="border-collapse:collapse;"><tr>
<td style="width:82px;vertical-align:top;">
<div style="width:72px;height:72px;border-radius:50%;background:${circleColor};text-align:center;line-height:72px;">
<img src="${brandSymbolWhiteUri}" style="width:44px;height:39px;vertical-align:middle;filter:brightness(0) invert(1);">
</div>
</td>
<td style="padding-left:12px;padding-top:6px;vertical-align:top;font-family:'Arial Narrow',Arial,sans-serif;color:#6b6b70;">
<div style="font-size:19px;font-weight:bold;line-height:1.1;">Jaqueline</div>
<div style="font-size:19px;font-weight:normal;line-height:1.1;">Vieira</div>
<div style="font-size:8px;font-weight:bold;margin-top:6px;">PSICÓLOGA&nbsp;&nbsp;CRP 06/191478</div>
</td>
</tr></table>
<div style="border-bottom:2px solid #9aa7aa;width:180px;margin-top:8px;"></div>
<div style="text-align:center;font-size:15px;font-weight:bold;color:#626267;margin:14px 0 0;font-family:'Arial Narrow',Arial,sans-serif;">${escHtml(title)}</div>
</div>`;
}
...
function documentPdfHtml(item){
  ...
  const body=`${campos}${secoes}${closing}`;
...
function prontuarioPdfHtml(nome,sessoes){
  ...
  const body=`<table class="pront-table"><thead><tr><th class="pront-data">Data</th><th class="pront-relato">Relato da sessão</th></tr></thead><tbody>${linhas}</tbody></table>`;
...
export async function renderDocumentoPdf(item){
  ...
  const colorido=(item.conteudo?.estilo||'colorido')==='colorido';
  return renderPdf(documentPdfHtml(item),{format:'A4',margin:PAGE_MARGIN,displayHeaderFooter:true,headerTemplate:headerTemplateHtml(item.titulo,colorido),footerTemplate:EMPTY_TEMPLATE});
}

export async function renderProntuarioPdf(nome,sessoes){
  const {renderPdf}=await import('./pdf.js');
  return renderPdf(prontuarioPdfHtml(nome,sessoes),{format:'A4',margin:PAGE_MARGIN,displayHeaderFooter:true,headerTemplate:headerTemplateHtml(`Prontuário — ${nome}`,true),footerTemplate:EMPTY_TEMPLATE});
}
