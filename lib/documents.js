...
function psmPdfHtmlKidsEmotion(individual,pacote){
  ...
  const slides=Array.from({length:10},(_,i)=>{
    ...
    const img=dataUri(readFileSync(new URL(`../assets/psm/infantil-emocoes/page-${n}.jpg`,import.meta.url)),'image/jpeg');
    let overlay='';
    if(i===1){overlay=`<div class="kids-overlay kids-card kids-tcc"><h2>Abordagem utilizada</h2><h3>Terapia Cognitivo-Comportamental</h3><div class="kids-body">${PSM_TEXT.infantil.abordagem.map(x=>`<p>${escHtml(x)}</p>`).join('')}</div></div>`;}
    else if(i===3){overlay=`<div class="kids-overlay kids-card kids-address"><h2>Terapia Presencial</h2><div class="kids-body">As sessões duram <b>50 min</b> e são realizadas no seguinte endereço:<br><br><strong>${escHtml(PSM_TEXT.infantil.endereco.nome)}</strong>${PSM_TEXT.infantil.endereco.linhas.map(x=>`<div>${escHtml(x)}</div>`).join('')}</div></div>`;}
    else if(i===4){overlay=`<div class="kids-overlay kids-card kids-rules"><h2>Observações agendamento</h2><ul>${PSM_TEXT.infantil.agendamento.map(x=>`<li>${escHtml(x)}</li>`).join('')}</ul></div>`;}
    else if(i===6){overlay=`<div class="kids-overlay kids-prices"><h2>Valores</h2><div class="kids-price-row"><div class="kids-price-card"><span>1 sessão</span><strong>${escHtml(valorBR(individual)||'R$ 0,00')}</strong></div><div class="kids-price-card"><span>Pacote com 4 sessões</span><strong>${escHtml(valorBR(pacote)||'R$ 0,00')}</strong></div></div><p>Pagamento · Pix: <b>57.062.579/0001-34</b><br>Gentileza enviar o comprovante até o dia da sessão.</p></div>`;}
    return `<section class="slide"><img src="${img}">${overlay}</section>`;
  }).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>PSM Infantil - Pacote ${escHtml(valorBR(pacote))}</title><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Baloo+2:wght@600;700;800&family=Nunito:wght@600;700;800&display=swap"><style>
@page{size:12in 8in;margin:0}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}body{margin:0}.slide{width:12in;height:8in;position:relative;overflow:hidden;break-after:page;background:#fff9f2}.slide:last-child{break-after:auto}.slide>img{width:100%;height:100%;object-fit:cover;display:block}.kids-overlay{position:absolute;z-index:2;font-family:'Nunito',sans-serif;color:#242022}.kids-card{background:rgba(255,249,242,.98);border-radius:.18in;box-shadow:0 .04in .14in rgba(81,51,31,.05)}.kids-overlay h2{font:800 25pt/1.06 'Baloo 2',cursive;color:#67438b;margin:0 0 .12in}.kids-overlay h3{font:800 16pt/1.2 'Nunito',sans-serif;color:#147f83;margin:0 0 .12in}.kids-body{font:700 10.5pt/1.48 'Nunito',sans-serif}.kids-body p{margin:0 0 .09in}.kids-tcc{left:.53in;top:1.46in;width:6.6in;padding:.24in .31in}.kids-address{left:.5in;top:1.88in;width:6.7in;padding:.28in .32in}.kids-address .kids-body{font-size:13pt;line-height:1.5}.kids-address strong{display:block;color:#67438b;font:800 16pt 'Baloo 2',cursive;margin-bottom:.09in}.kids-rules{left:.5in;top:1.56in;width:8.4in;padding:.25in .31in}.kids-rules ul{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:.11in}.kids-rules li{display:flex;gap:.12in;align-items:flex-start;font:700 10.2pt/1.42 'Nunito',sans-serif}.kids-rules li:before{content:'•';font-size:18pt;line-height:.75;color:#d67188}.kids-prices{left:1.98in;top:1.46in;width:8.04in;height:2.96in;background:#fff9f2;padding:.23in .36in;display:flex;flex-direction:column;justify-content:center}.kids-prices h2{text-align:center;margin-bottom:.13in}.kids-price-row{display:grid;grid-template-columns:1fr 1.18fr;gap:.22in}.kids-price-card{background:#fff0b8;border-radius:.18in;padding:.15in;text-align:center}.kids-price-card span{display:block;font:800 11pt/1.2 'Nunito',sans-serif;margin-bottom:.05in}.kids-price-card strong{display:block;font:800 19pt/1 'Baloo 2',cursive;color:#14827c}.kids-prices p{margin:.12in 0 0;text-align:center;font:700 10pt/1.35 'Nunito',sans-serif}.kids-prices p b{color:#14827c;font-size:11.5pt}
