...
const PSM_TEXT={
  ...
  infantil:{
    abordagem:[
      'Através da TCC será possível identificar as crenças disfuncionais que levam aos pensamentos mal adaptativos, influenciando as emoções e o comportamento do paciente.',
      'Ao longo das sessões, o objetivo é auxiliar o paciente a avaliar esses pensamentos de forma adaptativa, trazendo uma mudança das reações – comportamental, emocional e fisiológica.'
    ],
...
function psmPdfHtmlKidsApproved(individual,pacote){
  const valorBR=v=>{const n=Number(String(v||'').replace(',','.'));return Number.isFinite(n)?n.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'';};
  const slides=Array.from({length:10},(_,i)=>{
    const n=String(i+1).padStart(2,'0');
    const img=dataUri(readFileSync(new URL(`../assets/psm/infantil-emocoes/page-${n}.jpg`,import.meta.url)),'image/jpeg');
    const overlay=i===6?`<div class="ks-patch p1">${escHtml(valorBR(individual)||'R$ 0,00')}</div><div class="ks-patch p2">${escHtml(valorBR(pacote)||'R$ 0,00')}</div>`:'';
    return `<section class="psm-slide psm-kids-approved"><img src="${img}" alt="PSM infantil, página ${i+1}">${overlay}</section>`;
  }).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>PSM Infantil - Pacote ${escHtml(valorBR(pacote))}</title><style>
@font-face{font-family:AgrandirValues;src:url('${fontUri('agrandir-tight-values-bold.ttf')}');font-weight:700}
@page{size:12in 8in;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}
body{margin:0}
.psm-slide{width:12in;height:8in;position:relative;overflow:hidden;break-after:page}
.psm-slide:last-child{break-after:auto}
.psm-slide>img{width:100%;height:100%;object-fit:cover}
.psm-kids-approved .ks-patch{position:absolute;display:flex;align-items:center;justify-content:center;background:#f8edb1;color:#11837f;font:700 23pt AgrandirValues,Arial Narrow,sans-serif;letter-spacing:-.02em}
.psm-kids-approved .ks-patch.p1{left:25.8%;top:51.6%;width:22.3%;height:15.1%;border-radius:0 0 18pt 18pt}
.psm-kids-approved .ks-patch.p2{left:54.2%;top:51.6%;width:25.8%;height:15.1%;border-radius:0 0 18pt 18pt}
</style></head><body>${slides}</body></html>`;
}
...
export function psmPdfHtml(publico,individual,pacote,theme){
  if(publico==='infantil')return psmPdfHtmlKidsApproved(individual,pacote);
