...
export async function applySheetImport({supabaseUrl, supabaseKey, sheetUrl}) {
  ...
  try{
    ...
    // nome/ultima_chave/ultimo_label NÃO entram mais neste upsert em lote --
    // são campos do cadastro (fonte de verdade: /api/operational, save_patient)
    // e essa importação estava sobrescrevendo em massa, todo dia, qualquer
    // edição de nome completo feita no cadastro pelo nome abreviado que ainda
    // vem da planilha (relatado ao vivo: paciente editada na Agenda voltava a
    // aparecer só com o primeiro nome). "ativo" continua sincronizado da
    // planilha porque pacientes legados que nunca passaram pela Agenda
    // dependem dele.
    ...
    for(const item of prepared){
      ...
      const current=patientState.get(id)||{id,ativo:false};
      if(item.row.Ativo==='Ativo')current.ativo=true;
      patientState.set(id,current);
    }
    await B('pacientes?on_conflict=id',[...patientState.values()].map(p=>({...p,atualizado_em:new Date().toISOString()})));
