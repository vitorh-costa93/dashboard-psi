...
const TYPES=new Set(['termo_infantil','termo_adulto','orcamento','recibo','relatorio_psicologico','solicitacao_escolar','declaracao_comparecimento','declaracao_frequencia']);
...
export async function handleDocuments(req,res,user){
  try{
    ...
    if(req.method==='POST'&&req.body?.action==='generate'){
      const tipo=clean(req.body.tipo),descricao=clean(req.body.descricao);
      ...
      // psicóloga no formulário) -- "estruturado" divide em seções tituladas
      // (títulos livres, adaptados ao caso -- não mais um esqueleto fixo,
      // ver histórico do commit); "orientacao" é um texto corrido mais
      // curto, sem essa estrutura, pra quando o caso pede uma orientação
      // direta em vez de um relatório completo. O parser separarTextoIA no
      // frontend só quebra em seções quando o formato é estruturado, e usa
      // uma heurística (linha curta sem pontuação final = título) em vez de
      // uma lista fixa de palavras, pra acompanhar qualquer título que o
      // modelo escolha.
      const formato=clean(req.body.formato)==='orientacao'?'orientacao':'estruturado';
      // Calibrado em 12/09/2026 a partir de 5 relatórios reais dela e 9
      // conversas em que ela ajustou rascunhos do ChatGPT -- ver padrões
      // identificados: títulos livres por finalidade (nunca "Descrição/
      // Análise/Orientações" genéricos), sempre separar relato de terceiros
      // vs observação clínica vs hipótese, nunca linguagem absoluta
      // ("perfeito", "resolvido"), preservar fala textual da criança quando
      // tem valor clínico (só converter fala inadequada/coloquial em
      // discurso indireto), e nada de emoji/exclamação/comentário sobre o
      // texto -- só o relatório em si.
      const estiloRelatorio=' Organize o texto em prosa, dividido em 2 a 4 seções curtas com títulos livres e específicos à finalidade informada (nunca use "Descrição", "Análise" ou "Orientações" como título) -- por exemplo, um relatório de encaminhamento costuma ter "Desenvolvimento do acompanhamento", "Justificativa do encaminhamento" e "Fundamentação ética"; um relatório de evolução costuma ter só "Desenvolvimento do acompanhamento"; uma anamnese pode não precisar de nenhum título antes do primeiro parágrafo. Termine sempre com uma seção "Conclusão". Cada título fica sozinho em uma linha, sem dois pontos, seguido do parágrafo. Diferencie com clareza o que é relato da responsável/família, observação clínica direta em sessão e hipótese ou inferência da profissional -- nunca apresente um relato de terceiros como fato observado. Ao descrever evolução, equilibre avanços com dificuldades ainda presentes; nunca descreva o paciente como "perfeito", "resolvido" ou sem nenhuma dificuldade, mesmo em relatório de alta. Transforme falas coloquiais, ofensivas ou inadequadas em discurso indireto, técnico e respeitoso -- mas preserve entre aspas uma fala textual do próprio paciente quando ela tiver valor clínico direto (ex.: expressão de medo, sofrimento ou um pensamento relatado), em vez de sempre converter para discurso indireto. Nunca use emojis, pontos de exclamação, ou qualquer comentário sobre o texto (sugestões, perguntas, "se quiser posso..."); escreva apenas o relatório em si, sem nada antes ou depois.';
      const instructions=(tipo==='relatorio_psicologico'
        ? (formato==='orientacao'
          ? 'Redija uma orientação psicológica profissional em português do Brasil: um texto corrido, direto e curto (no máximo 3 a 4 parágrafos curtos), sem dividir em seções tituladas. Traga o contexto essencial em uma ou duas frases e o restante deve ser a orientação prática em si. Não invente diagnósticos, fatos ou dados. Use somente o relato fornecido. Transforme falas coloquiais, ofensivas ou citadas entre aspas em discurso indireto, técnico, respeitoso e adequado ao documento. Não reproduza insultos, palavrões ou falas literais entre aspas; descreva objetivamente o conteúdo e o contexto informado. Nunca use emojis, pontos de exclamação, ou qualquer comentário sobre o texto; escreva apenas a orientação em si.'
          : 'Redija um relatório psicológico profissional em português do Brasil. Não invente diagnósticos, fatos ou dados. Use somente o relato fornecido.'+estiloRelatorio)
