...
//
// Consequently the page itself carries NO top/side margin -- the brand strip
// must bleed to the very page edge, and a page margin would leave an unpainted
// white band above it. Side spacing comes from padding on the body cell, top
// spacing from the thead, and only the bottom margin stays a real page margin
// (nothing needs to bleed there, and it must repeat on every page).
const BRAND_STRIP_HEIGHT=108;
const BRAND_GAP=30;
const SIDE_PADDING=42;
export const PAGE_MARGIN={top:0,right:0,bottom:'30px',left:0};
...
table.doc-page>thead>tr>th.doc-head-cell{padding:0 0 ${BRAND_GAP}px;font-weight:inherit;text-align:left}
table.doc-page>tbody>tr>td.doc-body-cell{padding:0 ${SIDE_PADDING}px;text-align:left}
.doc-preview-brand{height:${BRAND_STRIP_HEIGHT}px;margin-left:${SIDE_PADDING}px;position:relative}
...
const pageTable=bodyHtml=>`<table class="doc-page"><thead><tr><th class="doc-head-cell">${brandStripMarkup()}</th></tr></thead><tbody><tr><td class="doc-body-cell">${bodyHtml}</td></tr></tbody></table>`;
