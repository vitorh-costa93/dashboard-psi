-- Bug real encontrado: finalizar_agenda_atendimento gravava
-- sessoes.comparecimento como 'Realizado'/'Falta'/'Cancelado', mas todo o
-- calculo do Dashboard (index.html: _compSim, _falta) so reconhece
-- exatamente 'Sim'/'Não' -- ou seja, todo atendimento liquidado pela Agenda
-- ficava fora da contagem de comparecimento/falta usada no saldo de
-- pacote, no KPI de Faltas e no painel "Faltas % por paciente". Esta
-- migration unifica o vocabulario em 'Sim'/'Não'/'Cancelado' (o mesmo que
-- a edição pelo Dashboard já usava) e corrige os registros ja gravados
...
BEGIN
  ...
  END IF;
  -- Vocabulario unificado com o que o Dashboard ja usa/entende: 'Sim'/'Não'/'Cancelado'.
...
-- Corrige os registros ja gravados com o vocabulario antigo (todo
-- atendimento liquidado pela Agenda ate agora).
UPDATE public.sessoes SET comparecimento='Sim' WHERE comparecimento='Realizado';
UPDATE public.sessoes SET comparecimento='Não' WHERE comparecimento='Falta';

-- Linha da Maitê em 02/09/2026 13:30 gerada por engano (confirmado pela
-- psicóloga: ela só deveria ter atendimento em 09/09/2026 nessa semana).
-- Escopo estreito de propósito: só apaga se ainda estiver "agendado" (sem
-- sessão vinculada), nunca um atendimento já liquidado.
DELETE FROM public.agenda_atendimentos
WHERE data_atendimento='2026-09-02' AND status='agendado' AND sessao_id IS NULL
  AND paciente_id=(SELECT id FROM public.pacientes WHERE nome ILIKE 'Maitê%' LIMIT 1);
