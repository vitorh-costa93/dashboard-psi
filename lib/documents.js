...
const divertidamente=readFileSync(new URL('../assets/psm/shared/divertidamente.jpg',import.meta.url));
...
const divertidamenteUri=dataUri(divertidamente,'image/jpeg');
...
// Kids variant: same skeleton and locked palette as the adult PSM (per the
// client's explicit constraint -- one deck, two skins, not two decks), but
// TCC for children starts by teaching kids to *name* a feeling, which is
// exactly what Divertidamente/Inside Out already dramatizes. So instead of
// pasting in more character art, the five feelings become one quiet
// structural mark reused on every kids slide: a row of five dots, each an
// existing brand color -- literally "this palette has room for every mood."
const PSM_MOOD='<span class="psm-mood"><i></i><i></i><i></i><i></i><i></i></span>';
function psmHeader(titulo,publico){
  const mood=publico==='infantil'?PSM_MOOD:'';
  return `<div class="psm-content-head"><span class="psm-mark">${PSM_SPRIG}</span><h2>${escHtml(titulo)}</h2>${mood}</div>`;
}
function psmWrap(publico){
  return publico==='infantil'?`psm-content psm-kids`:`psm-content`;
}
function psmAbordagemHtml(publico){
  const t=PSM_TEXT[publico]||PSM_TEXT.adulto;
  return `<div class="${psmWrap(publico)}"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Abordagem utilizada',publico)}
...
<img class="psm-abordagem-brain" src="${brainDiagramUri[publico]||brainDiagramUri.adulto}" alt="Como a TCC pode ajudar">
</div></div>`;
}
function psmOnlineHtml(publico){
  return `<div class="${psmWrap(publico)}"><img class="psm-leaf psm-leaf-bl" src="${leafBottomLeftUri}" alt="">${psmHeader('Terapia online',publico)}
...
function psmPresencialHtml(publico){
  const t=PSM_TEXT[publico]||PSM_TEXT.adulto;
  return `<div class="${psmWrap(publico)}"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Terapia presencial',publico)}
...
function psmListHtml(titulo,items,leaf,publico){
  ...
  const rot=[-6,4,-3,7,-5];
  return `<div class="${psmWrap(publico)}">${img}${psmHeader(titulo,publico)}
...
function psmValoresHtml(individual,pacote,publico){
  const valorBR=v=>{const n=Number(String(v||'').replace(',','.'));return Number.isFinite(n)?n.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'R$ 0,00';};
  return `<div class="${psmWrap(publico)}"><img class="psm-leaf psm-leaf-bl" src="${leafBottomLeftUri}" alt="">${psmHeader('Valores',publico)}
...
function psmAcompanheHtml(publico){
  return `<div class="${psmWrap(publico)}"><img class="psm-leaf psm-leaf-tr" src="${leafTopRightUri}" alt="">${psmHeader('Me acompanhe',publico)}
<div class="psm-photo-wrap"><img class="psm-photo" src="${wifePhotoUri}" alt="Jaqueline Vieira"><span class="psm-photo-mark">${PSM_SPRIG}</span></div>
...
function psmAgendamentoHtml(publico){
  return `<div class="${psmWrap(publico)}"><img class="psm-leaf psm-leaf-bl" src="${leafBottomLeftUri}" alt="">${psmHeader('Agendamento',publico)}
...
function psmInnerSlideHtml(index,publico,individual,pacote){
  ...
  if(index===4)return psmListHtml('Observações de agendamento',t.agendamento,'tr',publico);
  if(index===5)return psmListHtml('Observações de pagamento',PSM_PAGAMENTO,'bl',publico);
...
function psmCoverHtml(publico){
  ...
  const sticker=publico==='infantil'?`<div class="psm-cover-sticker"><img src="${divertidamenteUri}" alt=""><span class="psm-cover-sticker-tag">${PSM_SPRIG}</span></div>`:'';
  return `<div class="psm-cover-wrap">
<div class="psm-cover">
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700&family=Fraunces:ital,opsz,wght@0,9..144,300;0,9..144,400;0,9..144,500;0,9..144,600;1,9..144,300;1,9..144,400&family=Newsreader:ital,wght@0,400;0,500;1,400&family=DM+Sans:wght@400;500;700&display=swap">
<style>
...
.psm-cover-sticker{position:absolute;right:1in;bottom:.9in;transform:rotate(-6deg)}
.psm-cover-sticker img{width:1.7in;height:1.7in;border-radius:50%;object-fit:cover;border:1.5px solid #4e553e;display:block}
.psm-cover-sticker-tag{position:absolute;left:-.14in;bottom:-.05in;color:#b86a49;background:#f5f1e8;border-radius:50%;padding:.1in;display:flex;transform:rotate(-18deg)}
.psm-cover-sticker-tag svg{width:.24in;height:.24in;display:block}
...
.psm-content-head h2{font:300 36pt/1.15 'Fraunces',serif;font-style:italic;color:#4e553e;margin:0;letter-spacing:.002em}
...
.psm-mood{display:inline-flex;gap:.09in;margin-left:.1in;transform:translateY(-.02in)}
.psm-mood i{width:.13in;height:.13in;border-radius:50%;display:block}
.psm-mood i:nth-child(1){background:#4e553e}
.psm-mood i:nth-child(2){background:#899776}
.psm-mood i:nth-child(3){background:#b86a49}
.psm-mood i:nth-child(4){background:#d9cdb8}
.psm-mood i:nth-child(5){background:#f5f1e8;border:1px solid #c9b79a}
.psm-kids .psm-content-head h2{font-variation-settings:'opsz' 144;font-weight:500}
.psm-icon-badge{display:inline-flex;align-items:center;justify-content:center;color:#4e553e}
.psm-kids .psm-icon-badge{background:#f5f1e8;border:1.5px solid #d9cdb8;border-radius:50%;padding:.16in}
...
.psm-price-num{font:300 28pt 'Fraunces',serif;color:#4e553e}
...
.psm-photo{width:2.6in;height:2.6in;border-radius:50%;object-fit:cover;border:1.5px solid #4e553e}
.psm-photo-mark{position:absolute;right:calc(50% - 1.5in);bottom:.05in;color:#b86a49;background:#f5f1e8;border-radius:50%;padding:.12in;transform:rotate(14deg)}
.psm-photo-mark svg{width:.34in;height:.34in;display:block}
