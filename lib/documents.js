...
import {buscarPreferenciaTexto,aprenderComAjusteChat} from './aprendizado.js';
import {blocosDeTexto,paragrafosHtml,segmentosNegrito,prepararDeclaracao} from './doc-texto.js';

const TYPES=new Set(['termo_infantil','termo_adulto','orcamento','recibo','relatorio_psicologico','encaminhamento','solicitacao_escolar','declaracao','declaracao_comparecimento','declaracao_frequencia']);
...
p.par,p.lista{white-space:normal}
/* Recuo de primeira linha (tabulação de parágrafo) em todo documento; listas ficam sem recuo. */
p.par{text-indent:1.25cm}
p.campo{text-indent:0}
...
.doc-signature{position:relative;text-align:right;margin-top:130px;padding-top:14px;padding-right:92px;min-height:112px}
...
/* Termo de consentimento (infantil/adulto): assinatura da psicóloga e do
   responsável/paciente lado a lado, na mesma linha. */
.doc-signature-row{display:flex;justify-content:space-between;align-items:flex-end;gap:24px;margin-top:130px}
.doc-signature-row .doc-signature{margin-top:0}
.doc-signature-other{position:relative;text-align:left;padding-top:14px;min-height:112px}
.doc-signature-other:before{content:'';position:absolute;left:0;top:0;width:260px;border-top:2px solid #9aa7aa}
.doc-signature-other p{margin:0 0 2px;text-align:left;color:#3f3f44}
.doc-signature-other strong{color:#69696e}
</style>`;
}
...
export function documentPdfHtml(item){
  const c=item.conteudo||{},fields=c.campos||{},sections=item.tipo==='declaracao'?prepararDeclaracao(c.secoes,fields):(Array.isArray(c.secoes)?c.secoes:[]);
  const colorido=(c.estilo||'colorido')==='colorido';
  const campos=item.tipo==='declaracao'?'':Object.entries(fields).filter(([,v])=>clean(v)).map(([k,v])=>`<p class="campo"><strong>${escHtml(k)}: ${escHtml(dataBR(v))}</strong></p>`).join('');
  const secoes=sections.map(s=>`${s.titulo?`<h3>${escHtml(s.titulo)}</h3>`:''}${s.texto?paragrafosHtml(dataBR(s.texto),escHtml):''}`).join('');
  const psiSignature=`<div class="doc-signature"><img src="${brandSymbolUri}" alt=""><p><strong>Jaqueline Cristina Vieira</strong></p><p>CRP 06/191478</p><p>Psicóloga Clínica</p><p>Pós-graduada em Terapia Cognitivo-Comportamental</p></div>`;
  // Termo de consentimento (infantil/adulto): assinatura de quem consente --
  // o responsável na versão infantil, o próprio paciente na versão adulto --
  // fica na mesma linha da assinatura da psicóloga, não abaixo dela.
  const outroSignatario=item.tipo==='termo_infantil'?clean(fields['Responsável']):item.tipo==='termo_adulto'?clean(fields['Paciente']):'';
  const signatureBlock=outroSignatario?`<div class="doc-signature-row"><div class="doc-signature-other"><p><strong>${escHtml(outroSignatario)}</strong></p><p>${item.tipo==='termo_infantil'?'Responsável':'Paciente'}</p></div>${psiSignature}</div>`:psiSignature;
  const closing=`<div class="doc-closing"><p style="text-align:right;margin-top:24px">Araraquara, ${longDate(item.emitido_em)}.</p><p style="text-align:right">À disposição para esclarecimentos de quaisquer dúvidas.</p>${signatureBlock}</div>`;
...
// Parágrafo do DOCX com recuo de primeira linha e suporte a **negrito**.
function richParagraph(text,{indent=false,spaceAfter=150}={}){
  return new Paragraph({alignment:AlignmentType.JUSTIFIED,indent:indent?{firstLine:709}:undefined,spacing:{after:spaceAfter,line:300},children:segmentosNegrito(formatDates(text)).map(x=>new TextRun({text:x.text,bold:x.bold,font:'Arial',size:22,color:'222222'}))});
}

export function documentChildren(item){
  const c=item.conteudo||{}, fields=c.campos||{}, sections=item.tipo==='declaracao'?prepararDeclaracao(c.secoes,fields):(Array.isArray(c.secoes)?c.secoes:[]),accent=c.estilo==='preto-branco'?'222222':'596457';
  ...
  if(item.tipo!=='declaracao')Object.entries(fields).filter(([,v])=>clean(v)).forEach(([k,v])=>out.push(new Paragraph({spacing:{after:100},children:[new TextRun({text:`${k}: `,bold:true,font:'Arial',size:21}),new TextRun({text:formatDates(v),bold:true,font:'Arial',size:21})]})));
  sections.forEach(section=>{
    ...
    if(section.texto)blocosDeTexto(clean(section.texto)).forEach(b=>b.linhas.forEach((linha,i)=>out.push(richParagraph(linha,{indent:!b.lista&&i===0,spaceAfter:i===b.linhas.length-1?150:40}))));
  });
  ...
  out.push(paragraph('À disposição para esclarecimentos de quaisquer dúvidas.',{align:AlignmentType.RIGHT,spaceAfter:1300}));
  // Termo de consentimento (infantil/adulto): assinatura de quem consente --
  // o responsável na versão infantil, o próprio paciente na versão adulto --
  // fica na mesma linha da assinatura da psicóloga, não abaixo dela.
  const outroSignatario=item.tipo==='termo_infantil'?clean(fields['Responsável']):item.tipo==='termo_adulto'?clean(fields['Paciente']):'';
  const psiCells=[paragraph('Jaqueline Cristina Vieira',{bold:true,align:AlignmentType.RIGHT,spaceAfter:20,color:'66666B'}),paragraph('CRP 06/191478',{align:AlignmentType.RIGHT,spaceAfter:20}),paragraph('Psicóloga Clínica',{align:AlignmentType.RIGHT,spaceAfter:20}),paragraph('Pós-graduada em Terapia Cognitivo-Comportamental',{align:AlignmentType.RIGHT,spaceAfter:0})];
  const imageCell=new TableCell({width:{size:18,type:WidthType.PERCENTAGE},verticalAlign:VerticalAlign.BOTTOM,children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new ImageRun({data:brandSymbol,transformation:{width:67,height:75},type:'png',altText:{title:'Símbolo Jaqueline Vieira',description:'Flor da identidade visual',name:'Símbolo'}})]})]});
  if(outroSignatario){
    out.push(new Table({width:{size:100,type:WidthType.PERCENTAGE},borders:noBorders,rows:[new TableRow({children:[
      new TableCell({width:{size:39,type:WidthType.PERCENTAGE},verticalAlign:VerticalAlign.BOTTOM,children:[paragraph('________________________________',{color:'839297',spaceAfter:60}),paragraph(outroSignatario,{bold:true,spaceAfter:20,color:'66666B'}),paragraph(item.tipo==='termo_infantil'?'Responsável':'Paciente',{spaceAfter:0})]}),
      new TableCell({width:{size:43,type:WidthType.PERCENTAGE},verticalAlign:VerticalAlign.BOTTOM,children:[paragraph('________________________________________',{align:AlignmentType.RIGHT,color:'839297',spaceAfter:60}),...psiCells]}),
      imageCell,
    ]})]}));
  } else {
    out.push(paragraph('________________________________________',{align:AlignmentType.RIGHT,spaceAfter:60,color:'839297'}));
    out.push(new Table({width:{size:100,type:WidthType.PERCENTAGE},borders:noBorders,rows:[new TableRow({children:[
      new TableCell({width:{size:82,type:WidthType.PERCENTAGE},verticalAlign:VerticalAlign.CENTER,children:psiCells}),
      imageCell,
    ]})]}));
  }
  return out;
...
export async function handleDocuments(req,res,user){
  try{
    ...
    if(req.method==='POST'&&req.body?.action==='generate'){
      ...
      const contextoDeclaracao=tipo==='declaracao'&&req.body.contextoDeclaracao&&typeof req.body.contextoDeclaracao==='object'
        ?{paciente:clean(req.body.contextoDeclaracao.paciente).slice(0,120),frequencia:clean(req.body.contextoDeclaracao.frequencia).slice(0,80),inicio:clean(req.body.contextoDeclaracao.inicio_atendimentos).slice(0,10)}
        :null;
      ...
      const instructions=(tipo==='relatorio_psicologico'
        ? (formato==='orientacao'
          ? 'Redija uma orientação psicológica profissional em português do Brasil, direta e objetiva. Comece com uma ou duas frases de contexto (o que motivou a orientação) seguidas de uma frase dizendo que as orientações são práticas e serão ajustadas conforme os relatos dos responsáveis e professores. Quando a orientação envolver mais de um ambiente ou público (ex.: escola e família), organize em blocos curtos com um rótulo simples seguido de vírgula (ex.: "Na escola," e "Em casa,") e liste cada recomendação como item numerado (1), 2), 3)...), um por linha -- nunca como parágrafo corrido nesse caso. Se envolver só um público, mantenha texto corrido curto (3 a 4 parágrafos no máximo), sem seções tituladas. Em qualquer um dos dois formatos, cada recomendação deve ser UMA instrução curta e concreta: nunca inclua exemplos entre parênteses, falas ou frases-modelo entre aspas, nem explicação teórica da técnica -- vá direto à ação (ela mesma decide como aplicar). Não acrescente recomendação que a descrição não pediu, como sugerir avaliação multiprofissional sem isso ter sido indicado no relato. Feche com uma frase curta se colocando à disposição para ajustar as orientações conforme o retorno da escola/família (não repita "à disposição para esclarecimentos", isso já está no fechamento fixo do documento). Não invente diagnósticos, fatos ou dados. Use somente o relato fornecido.'+linguagemRelatorio
        ...
        : tipo==='declaracao'
        ? `Redija somente o corpo de uma declaração psicológica de continuidade de acompanhamento, em português do Brasil. A abertura com identificação da psicóloga, nome do paciente, início e frequência será inserida pelo aplicativo; portanto, não repita essa abertura, nem crie título, assinatura, data ou despedida. Use exclusivamente os fatos trazidos no relato. Não invente diagnósticos, sintomas, encaminhamentos, idade, objetivos ou evolução. Escreva em 2 a 5 parágrafos claros, formais e humanos, adequados a uma declaração profissional.${contextoDeclaracao?` Contexto obrigatório já informado no documento: paciente ${contextoDeclaracao.paciente||'não informado'}; frequência ${contextoDeclaracao.frequencia||'não informada'}; início ${contextoDeclaracao.inicio||'não informado'}.`:''}`+tomHumanizado
