...
async function psmInfantilWebpPdf(individual=130,pacote=480){
  const pdf=await PDFDocument.create();
  const formatValor=v=>{const n=Number(String(v??'').replace(',','.'));return Number.isFinite(n)?n.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'R$ 0,00';};
  for(let i=1;i<=10;i++){
    const n=String(i).padStart(2,'0');
    const webp=readFileSync(new URL(`../assets/psm/infantil/emocoes-page-${n}.webp`,import.meta.url));
    let imageSource=webp;
    if(i===7){
      const svg=`<svg width="2430" height="1620" xmlns="http://www.w3.org/2000/svg">
        <style>
          .v{font-family:"DejaVu Sans",Arial,sans-serif;font-size:105px;font-weight:700;fill:#087f83;letter-spacing:0}
        </style>
        <text class="v" x="650" y="640" text-anchor="middle">${formatValor(individual)}</text>
        <text class="v" x="1510" y="640" text-anchor="middle">${formatValor(pacote)}</text>
      </svg>`;
      imageSource=await sharp(webp).composite([{input:Buffer.from(svg),top:0,left:0}]).webp({quality:100}).toBuffer();
    }
    const jpg=await sharp(imageSource).jpeg({quality:95}).toBuffer();
