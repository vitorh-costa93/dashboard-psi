...
function psmPdfHtmlKidsEmotion(individual,pacote){
  const asset=name=>dataUri(readFileSync(new URL(`../assets/psm/infantil/${name}`,import.meta.url)),'image/webp');
  const slides=Array.from({length:10},(_,i)=>{
    const n=String(i+1).padStart(2,'0');
    const img=asset(`emocoes-page-${n}.webp`);
    const overlay=i===6?`<div class="psm-kids-values"><div class="psm-kids-value one">${escHtml(valorBR(individual)||'R$ 0,00')}</div><div class="psm-kids-value four">${escHtml(valorBR(pacote)||'R$ 0,00')}</div></div>`:'';
    return `<section class="psm-slide"><img src="${img}" alt="PSM infantil, página ${i+1}">${overlay}</section>`;
  }).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>PSM Infantil - Pacote ${escHtml(valorBR(pacote)||'R$ 0,00')}</title><style>
@font-face{font-family:'Agrandir Values';src:url('${fontUri('agrandir-tight-values-bold.ttf')}');font-weight:700}
@page{size:12in 8in;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}
html,body{margin:0;padding:0;background:#fff}
.psm-slide{width:12in;height:8in;position:relative;overflow:hidden;break-after:page;background:#fbfaf9}
.psm-slide:last-child{break-after:auto}
.psm-slide>img{display:block;width:100%;height:100%;object-fit:contain}
.psm-kids-values{position:absolute;inset:0;pointer-events:none}
.psm-kids-value{position:absolute;color:#0a837f;font:700 31pt 'Agrandir Values','Arial Narrow',sans-serif;line-height:1;letter-spacing:-.02em;text-align:center;white-space:nowrap}
...
function psmStaticFolder(publico){return publico==='infantil'?'infantil-emocoes':publico;}
...
export function psmPdfHtml(publico,individual,pacote,theme){
  if(theme==='legacy')return psmPdfHtmlLegacy(publico,individual,pacote);
