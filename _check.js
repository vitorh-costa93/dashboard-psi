...
function analisarZonaTexto(b64, zone){
  return new Promise((resolve)=>{
    ...
    img.onload=()=>{
      ...
      const w=img.naturalWidth||1024,h=img.naturalHeight||1024;
      const map={
        left:{x:.075,y:.20,w:.49,h:.60},
        right:{x:.43,y:.20,w:.49,h:.60},
        center:{x:.17,y:.22,w:.66,h:.56}
      };
      const z=map[zone]||map.left;
      ctx.drawImage(img,z.x*w,z.y*h,z.w*w,z.h*h,0,0,64,64);
      const d=ctx.getImageData(0,0,64,64).data;
      let sum=0,sumSq=0,n=0;
      for(let i=0;i<d.length;i+=4){const a=d[i+3]/255;if(a<.9)continue;const lum=luminancia([d[i],d[i+1],d[i+2]]);sum+=lum;sumSq+=lum*lum;n++;}
      const avg=n?sum/n:.7, variance=n?Math.max(0,sumSq/n-avg*avg):.03;
      const homogeneity=Math.max(0,1-Math.sqrt(variance)*4);
      const darkRatio=textoKontrastRatio(BRAND_INK,avg);
      const lightRatio=textoKontrastRatio(BRAND_CREAM,avg);
      const best=Math.max(darkRatio,lightRatio);
      resolve({zone,avg,homogeneity,darkRatio,lightRatio,best,preferred:darkRatio>=lightRatio?'dark':'light'});
    };
    img.onerror=()=>resolve({zone,avg:.7,homogeneity:.5,darkRatio:5,lightRatio:3,best:5,preferred:'dark'});
    img.src='data:image/png;base64,'+b64;
  });
}
...
async function escolherLayoutEditorial(imagens){
  ...
  for(const zone of textCandidates){
    ...
    const avgContrast=stats.reduce((a,z)=>a+z.best,0)/stats.length;
    const minContrast=Math.min(...stats.map(z=>z.best));
    const avgHom=stats.reduce((a,z)=>a+z.homogeneity,0)/stats.length;
    const lightCount=stats.filter(z=>z.preferred==='light').length;
    // Favorece legibilidade consistente e espaço negativo, com uma preferência leve pela esquerda.
    const preference=zone==='left'?0.16:(zone==='right'?0.03:0);
    const score=(Math.min(avgContrast,10)*.5)+(Math.min(minContrast,10)*.35)+(avgHom*.15)+preference;
    if(!bestText||score>bestText.score)bestText={zone,score,stats,avgContrast,minContrast,avgHom,lightCount};
  }

  // A cor do texto é única no carrossel, determinada pela zona editorial comum.
  // Se a zona mistura claros/escuros, preferimos texto escuro e reforçamos o fundo com um véu sutil.
  const textColor=bestText.lightCount>imagens.length/2?BRAND_CREAM:BRAND_INK;

  // Logo: escolhe o canto oposto à área de texto, privilegiando o melhor contraste conjunto.
  const logoCandidates=['tr','br','tl','bl'];
  const penalty={tr:0,br:0,tl:.05,bl:.04};
  let bestLogo=null;
  for(const pos of logoCandidates){
    const zonas=await Promise.all(imagens.map(b=>analisarZonaLogo(b,pos)));
    const mediaLum=zonas.reduce((a,z)=>a+z.avg,0)/zonas.length;
    const mediaHom=zonas.reduce((a,z)=>a+z.homogeneity,0)/zonas.length;
    const contrastDark=textoKontrastRatio(BRAND_DEEP,mediaLum);
    const contrastLight=textoKontrastRatio(BRAND_CREAM,mediaLum);
    const logoColor=contrastDark>=contrastLight?BRAND_DEEP:BRAND_CREAM;
    const logoContrast=Math.min(contrastDark,contrastLight);
    const oppositeText=(bestText.zone==='left' && (pos==='tr'||pos==='br')) || (bestText.zone==='right' && (pos==='tl'||pos==='bl')) ? .25 : 0;
    const score=Math.min(logoContrast,10)*.58+mediaHom*.24+oppositeText-penalty[pos];
    if(!bestLogo||score>bestLogo.score)bestLogo={position:pos,color:logoColor,score,mediaLum,contrast:logoContrast};
  }
...
async function aplicarLogoPost(b64, corPreferida, pageInfo=null, logoLayout=null){
  return new Promise((resolve,reject)=>{
    ...
    base.onload=()=>{logo.onload=()=>{
      ...
      // Logo real, com transparência original e sem caixa de fundo.
      ctx.drawImage(off,x,y,targetW,targetH);

      if(pageInfo?.current && pageInfo?.total){
        const label=`${pageInfo.current}/${pageInfo.total}`;
        ctx.save();
        ctx.font='600 '+Math.max(18,Math.round(canvas.width*.026))+'px "Source Sans 3", sans-serif';
        ctx.textAlign='left';ctx.textBaseline='bottom';
        const margin=Math.max(18,Math.round(canvas.width*.04));
        ctx.fillStyle=corPreferida||BRAND_DEEP;
        ctx.fillText(label,margin,canvas.height-margin);
        ctx.restore();
      }
...
function renderArtesPost(){
  const wrap=document.getElementById('post-img-wrap');if(!_postAtual?.imagens_b64?.length){wrap.innerHTML='<div class="preview-placeholder">As artes serão geradas depois que o conteúdo for aprovado.</div>';return;}
...
function hexToRgb(hex){
  const m=(hex||'').replace('#','').match(/^([0-9a-f]{6})$/i); if(!m)return [137,151,118];
  return [parseInt(m[1].slice(0,2),16),parseInt(m[1].slice(2,4),16),parseInt(m[1].slice(4,6),16)];
}
function luminancia(rgb){
  const v=rgb.map(x=>{x/=255;return x<=.03928?x/12.92:Math.pow((x+.055)/1.055,2.4)});
...
function desenharVéuEditorial(ctx, canvas, zone, textColor){
  const zones={
    left:{x:.055,y:.16,w:.55,h:.68},
    right:{x:.395,y:.16,w:.55,h:.68},
    center:{x:.14,y:.18,w:.72,h:.62}
  };
  ...
  if(textColor===BRAND_CREAM){
    grad.addColorStop(0,'rgba(56,62,48,.74)');
    grad.addColorStop(.72,'rgba(56,62,48,.30)');
    grad.addColorStop(1,'rgba(56,62,48,0)');
  }else{
    grad.addColorStop(0,'rgba(244,241,232,.90)');
    grad.addColorStop(.72,'rgba(244,241,232,.56)');
    grad.addColorStop(1,'rgba(244,241,232,0)');
  }
  ctx.save();ctx.fillStyle=grad;ctx.fillRect(x,y,w,h);ctx.restore();
  return {x:x+w*.08,y:y+h*.16,w:w*.80};
}
...
function desenharTextoArte(b64, slideText, titulo, isCarousel, index, total, editorialLayout){
  return new Promise((resolve,reject)=>{
    const img=new Image(); img.onload=async()=>{
      ...
      const accent=layout.logoColor||BRAND_SAGE;
      const fg=layout.textColor||BRAND_INK;
      const zoneBox=desenharVéuEditorial(ctx,canvas,layout.textZone,fg);
      const maxW=Math.round(zoneBox.w), marginX=zoneBox.x;
      ctx.textAlign='left';
      ctx.textBaseline='top';
      if(isCarousel && index===0){
        ctx.fillStyle=accent;
        ctx.font=`700 ${Math.round(canvas.width*.064)}px "Newsreader", serif`;
        const titleLines=wrapText(ctx,titulo,maxW); let y=zoneBox.y;
        titleLines.slice(0,4).forEach(line=>{ctx.fillText(line,marginX,y);y+=Math.round(canvas.width*.074);});
        ctx.fillStyle=fg; ctx.font=`500 ${Math.round(canvas.width*.032)}px "Source Sans 3", sans-serif`;
        const sub=wrapText(ctx,slideText,maxW); y+=Math.round(canvas.width*.018); sub.slice(0,8).forEach(line=>{ctx.fillText(line,marginX,y);y+=Math.round(canvas.width*.043);});
      }else{
        ctx.fillStyle=accent; ctx.font=`700 ${Math.round(canvas.width*.047)}px "Newsreader", serif`;
        const lines=wrapText(ctx,slideText,maxW); let y=zoneBox.y;
        lines.slice(0,10).forEach(line=>{ctx.fillText(line,marginX,y);y+=Math.round(canvas.width*.056);});
      }
      resolve(canvas.toDataURL('image/png').split(',')[1]);
...
async function gerarArtePost(){
  ...
  const formato=_postAtual.formato||'Carrossel', isCarousel=formato.toLowerCase()==='carrossel', rawSlides=Array.isArray(_postAtual.slides)?_postAtual.slides.filter(Boolean).map(x=>String(x).trim()).filter(Boolean):[];
  // Para Post/Story, não bloqueie a geração só porque a IA retornou slides vazios.
  // O conteúdo pode estar em gancho, título ou CTA; o aplicativo normaliza isso aqui.
  const fallback=[_postAtual.gancho,_postAtual.titulo,_postAtual.cta,_postAtual.legenda].map(x=>String(x||'').trim()).filter(Boolean);
  const slides=isCarousel?rawSlides.slice(0,7):rawSlides.length?rawSlides.slice(0,4):fallback.slice(0,4);
  if(isCarousel && slides.length!==7){toast('O carrossel precisa ter exatamente 7 slides. Gere o conteúdo novamente.');return;}
  if(!isCarousel && !slides.length){toast('Não há conteúdo suficiente para gerar a arte. Descreva melhor o tema ou gere o conteúdo novamente.');return;}
  try{
    const wrap=document.getElementById('post-img-wrap');wrap.innerHTML='<div class="inline-loading"><div class="inline-spinner"></div> Preparando artes...</div>';
    _postAtual.imagens_b64=[];
    let identidadeCor=BRAND_SAGE;
    const fundos=[];
    for(let i=0;i<slides.length;i++){
      wrap.innerHTML=`<div class="inline-loading"><div class="inline-spinner"></div> Gerando fundo ${i+1} de ${slides.length}...</div>`;
      const prompt=`Create ONE single clean visual background for an Instagram ${formato} slide, slide ${i+1} of ${slides.length}, for a Brazilian psychologist who works across the life cycle. Theme: ${_postAtual.tema}. Audience: ${_postAtual.publico||'público geral'}. Visual concept related to this slide: ${slides[i]}. ${isCarousel&&i===0?`This is the cover; create a strong but calm editorial composition suitable for the title: ${_postAtual.titulo||''}.`:''} Elegant editorial psychology brand using the Jaqueline Vieira visual system: warm cream/off-white #F4F1E8, sage green #899776, deep sage #687257, warm sand #D8CDB8 and restrained terracotta #C5654E. Organic shapes, generous negative space, refined editorial composition, quiet luxury, professional and welcoming. NO text, NO letters, NO words, NO typography, NO logo, NO names, NO credentials, NO page numbers, NO UI, NO watermark. Leave generous negative space for text overlay and allow the composition to work with branding in any corner; do not reserve a fixed logo corner. The application will choose the safest common logo position and contrast after all slides are generated. High visual coherence across a 7-slide carousel; do not invent written content.`;
      fundos.push(await gerarImagem(prompt));
    }
    // O layout editorial é decidido em conjunto para TODAS as imagens.
    // Isso garante uma única posição de texto, uma única cor tipográfica e
    // uma única posição/cor de logo para todo o carrossel.
    const editorialLayout=await escolherLayoutEditorial(fundos);
    identidadeCor=editorialLayout.logoColor;
    _postAtual.logo_posicao=editorialLayout.logoPosition;
    _postAtual.logo_contraste=editorialLayout.logoContrast;
    _postAtual.texto_posicao=editorialLayout.textZone;
    _postAtual.texto_cor=editorialLayout.textColor;
    _postAtual.texto_contraste=editorialLayout.textContrast;
    for(let i=0;i<fundos.length;i++){
      wrap.innerHTML=`<div class="inline-loading"><div class="inline-spinner"></div> Aplicando identidade visual ${i+1} de ${fundos.length}...</div>`;
      const finalB64=await aplicarLogoPost(fundos[i],identidadeCor,isCarousel?{current:i+1,total:slides.length}:null,editorialLayout);
      const composed=await desenharTextoArte(finalB64,slides[i],_postAtual.titulo||'',isCarousel,i,slides.length,editorialLayout);
      _postAtual.imagens_b64.push(composed); _postAtual.logo_cor=identidadeCor; renderArtesPost();
    }
    _postAtual.img_b64=_postAtual.imagens_b64[0]||null;
    _postAtual.prompt='Fundo gerado por IA; tipografia, paginação e identidade visual aplicadas deterministicamente pelo aplicativo.';
  }catch(e){toast('Erro ao gerar arte: '+e.message);if(_postAtual.imagens_b64?.length)renderArtesPost();}
}
