...
// nome_exibicao_paciente() e do helper equivalente em api/operational.js e
// index.html).
...
export async function applySheetImport({supabaseUrl, supabaseKey, sheetUrl}) {
  ...
  try{
    ...
    for(const item of prepared){
      ...
      if(item.row.Ativo==='Ativo'){current.ativo=true;current.ultimo_label=`${nomeExibicao(item.row.Paciente)}${item.row['Horário']?' | '+item.row['Horário']:''}`;}
